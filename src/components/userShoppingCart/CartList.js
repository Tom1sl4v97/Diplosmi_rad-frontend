...
function CartList(props) {
  ...
  const [totalItems, setTotalItems] = useState(userCart.map((item) => item.quantityInCart).reduce((a, b) => a + b, 0));
  const [totalPrice, setTotalPrice] = useState(calulateTotalPrice());

  const addTotalItems = (price) => {
    setTotalItems((prevTotalItems) => prevTotalItems + 1);
    setTotalPrice((prevTotalPrice) => prevTotalPrice + price);
  };

  const removeTotalItems = (price) => {
    setTotalItems((prevTotalItems) => prevTotalItems - 1);
    setTotalPrice((prevTotalPrice) => prevTotalPrice - price);
  };

  const onChangeTotalItems = (price, quantity) => {
  ...
  const onRemoveItemHandler = (id) => {
    setUserCart((prevUserCart) =>
      prevUserCart.filter((item) => item.id !== id)
    );
  };
